...
def split_records(page_text: str) -> list[str]:
    ...
    return [
        part.strip()
        for part in parts
        if re.search(r"Protocol[oa]?b?\s+(?:AT\s*-?\s*\d{3}|PROTOCOLO\s*\??)", part, re.I)
    ]
...
def has_ocr_failure(session, doc: Documento) -> bool:
    return session.scalar(
        select(ErroProcessamento.id).where(
...
def purge_documento(session, doc: Documento) -> None:
    session.execute(delete(ErroProcessamento).where(ErroProcessamento.documento_id == doc.id))
    session.delete(doc)
    session.flush()
...
def ingest_pages(session, pdf: Path, doc: Documento, page_data: list[dict], cfg: dict, categories: dict, rows: list[dict], persist: bool) -> None:
    for page in page_data:
        text = page["texto"]
        raw_text = text
        if page["metodo"] == "ocr_pendente":
            try:
                raw_text = ocr_page(pdf, page["pagina"], cfg["ocr"]["dpi"], cfg["ocr"]["idioma"])
                text = repair_ocr_text(raw_text)
                page["metodo"] = "ocr"
                logging.info(
                    "OCR concluído: %s p.%s bruto=%s chars limpo=%s chars",
                    pdf.name, page["pagina"], len(raw_text), len(text),
                )
            except Exception as exc:
                message = f"{pdf.name}: {exc}"
                if persist:
                    session.add(ErroProcessamento(
                        documento_id=doc.id, pagina=page["pagina"], etapa="ocr",
                        tipo=type(exc).__name__, mensagem=message,
                    ))
                logging.error("OCR falhou: %s p.%s — %s", pdf.name, page["pagina"], exc)
                continue
        logging.info("Extração registrada: documento=%s pagina=%s metodo=%s", pdf.name, page["pagina"], page["metodo"])
        for raw in split_records(text):
            ...
            if classification == "duplicado":
                session.add(ErroProcessamento(
                    documento_id=doc.id, pagina=page["pagina"], etapa="deduplicacao",
                    tipo="Duplicidade", mensagem=protocol,
                ))
                continue
            item = Atendimento(
                documento_id=doc.id, pagina=page["pagina"], protocolo=protocol,
                data=normalized.get("data_obj"), solicitante=fields.get("solicitante"),
                email=fields.get("email"), categoria=row["categoria"],
                descricao=fields.get("descricao"), solucao=fields.get("solucao"),
                tempo_minutos=normalized.get("tempo_obj"), status=status,
                cep=fields.get("cep"), municipio=None, uf=None,
                classificacao=classification, motivos=row["motivos"],
                texto_original=raw, texto_limpo=preprocess(raw),
            )
            session.add(item)
            session.flush()
            for idx, content in enumerate(split_chunks(raw, cfg["embeddings"]["tamanho_chunk"], cfg["embeddings"]["sobreposicao"])):
                ...
                session.add(Chunk(
                    atendimento_id=item.id, documento_id=doc.id, pagina=page["pagina"],
                    indice=idx, conteudo=content, metadata_json=metadata_json(**meta),
                ))
...
def process_all(cfg: dict) -> pd.DataFrame:
    ...
    with session_scope(factory) as session:
        for pdf in sorted(pdf_dir.glob(cfg["entrada"]["padrao"])):
            ...
            existing = session.scalar(select(Documento).where(Documento.hash_sha256 == digest))
            if existing and has_ocr_failure(session, existing):
                ...
                existing = None
            if existing:
                logging.info("Documento já processado; reutilizando registros: %s", pdf.name)
                rows.extend(atendimento_to_row(item, pdf.name, existing.metodo) for item in existing.atendimentos)
                if not existing.atendimentos:
                    ingest_pages(session, pdf, existing, page_data, cfg, categories, rows, persist=False)
                else:
                ...
                continue
            method = "ocr" if all(page["metodo"] == "ocr_pendente" for page in page_data) else "extracao_direta"
            doc = Documento(nome_arquivo=pdf.name, hash_sha256=digest, total_paginas=len(page_data), metodo=method)
            session.add(doc)
            session.flush()
            ingest_pages(session, pdf, doc, page_data, cfg, categories, rows, persist=True)
