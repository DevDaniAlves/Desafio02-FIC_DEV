"""Extração por regex, normalização e validação dos registros (RF04)."""
...
EMPTY_MARKERS = {"", "vazio", "n/a", "na", "-", "none", "null", "sem informacao"}
FIELD_PATTERNS = {
    "protocolo": [
    ...
    "tempo_minutos": [r"Tempo\s+(-?\d+)\s*min", r"Tempo\s+(-?\d+)?\s*min"],
...
def is_missing(value: str | None) -> bool:
    ...
    if not raw:
        return True
    return normalize_key(raw).strip("[](){}") in EMPTY_MARKERS

def normalize_protocol(value: str) -> str:
    value = (value or "").strip().upper()
    match = re.search(r"AT\s*-?\s*(\d{3})", value)
    if match:
        return f"AT-{match.group(1)}"
    if "PROTOCOLO" in value:
        return "PROTOCOLO?"
    return value
...
def normalize_category(value: str, categories: dict) -> str | None:
    ...
    for item in categories.get("categorias_oficiais", []):
        aliases = {normalize_key(item["nome"]), *(normalize_key(v) for v in item["variacoes"])}
        if target in aliases:
            return item["nome"]
    return None

def normalize_status(value: str, categories: dict) -> str | None:
    target = normalize_key(value)
    for item in categories.get("status_validos", ["Concluido", "Pendente", "Em atendimento"]):
        if target == normalize_key(item):
            return item
    return None
...
def validate_record(record: dict, categories: dict) -> tuple[str, list[str], dict]:
    r = dict(record)
    reasons = []
    protocol = normalize_protocol(r.get("protocolo", ""))
    r["protocolo"] = protocol
    if not PROTO_RE.fullmatch(protocol):
        reasons.append("protocolo_invalido")
    r["data_obj"] = parse_date(r.get("data", ""))
    if not r["data_obj"]:
        reasons.append("data_invalida")
    if not EMAIL_RE.fullmatch(r.get("email", "")):
    ...
    r["status_normalizado"] = normalize_status(r.get("status", ""), categories)
    if r.get("status") and not r["status_normalizado"]:
        reasons.append("status_invalido")
    try:
